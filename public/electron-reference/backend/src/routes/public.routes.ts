...
router.get('/verify-availability', async (req, res, next) => {
  try {
    const { domain, email } = req.query;

    if (domain) {
      const existingDomain = await Company.findOne({ domain: (domain as string).toLowerCase() });
      if (existingDomain) {
    ...
    if (email) {
      const existingEmail = await User.findOne({ email: (email as string).toLowerCase() });
      if (existingEmail) {
